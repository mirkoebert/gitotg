...
public class AdvisorService {
    ...
    public List<String> getAdviseList(@NonNull final String userId){
        int c = hcpRepository.countByUserId(userId);
        c = c  + singleTestResultRepository.countByUserId(userId);
        ...
        if (c < 5){
            log.info("fresh");
            return List.of(message(FRESH_KEYS[r.nextInt(FRESH_KEYS.length)]));
        } else if (c < 25) {
            log.info("newby");
            return List.of(message(FEW_KEYS[r.nextInt(FEW_KEYS.length)]));
        } else if (hcp.isPresent() && HandicapClassifier.HIGH_HANDICAPER.equals(handicapClassifier.apply(hcp.get().getHcp()))) {
            log.info(HandicapClassifier.HIGH_HANDICAPER);
            return List.of(message(HH_KEYS[r.nextInt(HH_KEYS.length)]));
        }
        // analyze hcp
        // analyze sgi
        log.info("other");
        return List.of(message(OTHER_KEYS[r.nextInt(OTHER_KEYS.length)]));
    }
